...
-- 한 달에 여러 채널을 운영할 수 있도록 구조 변경 + 성과를 채널별로 기록.
--   ① monthly_marketing_channels 생성 (채널 1개 = 행 1개)
--   ② 기존 monthly_channel_info / applications 데이터 백필
--   ③ monthly_performance 에 slot 추가 (한 달 여러 성과 허용)
-- 기존 표와 데이터는 지우지 않습니다. 멱등(재실행 안전).
-- ─────────────────────────────────────────────────────────────────────────────
